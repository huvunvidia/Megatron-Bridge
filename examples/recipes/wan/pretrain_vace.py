...
"""
VACE Finetuning Script with YAML and CLI Configuration Overrides.

This script provides a flexible way to pretrain VACE models using Megatron-Bridge with support for
both YAML configuration files and command-line overrides using Hydra-style syntax.

Examples:
    Basic usage with default configuration:
        $ torchrun --nproc_per_node=8 pretrain_vace.py

    Using a custom YAML config file:
        $ torchrun --nproc_per_node=8 pretrain_vace.py --config-file my_custom_config.yaml

    Using CLI overrides only:
        $ torchrun --nproc_per_node=8 pretrain_vace.py model.tensor_model_parallel_size=4 train.train_iters=100000

    Combining YAML and CLI overrides (CLI takes precedence):
        $ torchrun --nproc_per_node=8 pretrain_vace.py --config-file conf/my_config.yaml \
        model.pipeline_dtype=torch.float16 \
        train.global_batch_size=512

Configuration Precedence:
    1. Base configuration from vace_pretrain_config() recipe
...
# Assumes this script (pretrain_vace.py) is in Megatron-Bridge/examples/recipes/wan/
# and the config is in a 'conf' subdirectory.
SCRIPT_DIR: Path = Path(__file__).parent.resolve()
DEFAULT_CONFIG_FILENAME: str = "vace_pretrain_override_example.yaml"
...
def parse_cli_args() -> Tuple[argparse.Namespace, list[str]]:
    ...
    parser = argparse.ArgumentParser(
        description="pretrain VACE model using Megatron-Bridge with YAML and CLI overrides",
    ...
    parser.add_argument(
        ...
        help="Path to the YAML OmegaConf override file. Default: conf/vace_pretrain_override_example.yaml",
    )
...
def main() -> None:
    """
    Entry point for the VACE finetuning script.

    This function orchestrates the complete configuration workflow:
    1. Loads the base configuration from vace_pretrain_config() recipe
    2. Applies YAML overrides from --config-file (if exists)
    3. Applies CLI overrides using Hydra-style syntax
    4. Starts Megatron finetuning with the final merged configuration
    ...
    Examples of CLI usage:
        ...
        torchrun --nproc_per_node=8 pretrain_vace.py optimizer.lr=0.0002

        # Custom config file with additional overrides
        torchrun --nproc_per_node=8 pretrain_vace.py --config-file my_config.yaml train.train_iters=50000
    ...
    logger.info("Megatron-Bridge VACE Finetuning Script with YAML & CLI Overrides")
    logger.info("------------------------------------------------------------------")

    # Load base configuration from the recipe as a Python dataclass
    cfg: ConfigContainer = vace_pretrain_config()
    logger.info("Loaded base configuration for VACE finetuning")
